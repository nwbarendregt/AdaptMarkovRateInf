% tsb_adapt_sampled_rate_performance.m
% Calculates performance metrics for adaptive inference of two
% transition rates, averaged over pairs of transition rates, for different
...
% Set posterior covariance determinant convergence tolerances:
Cov_tol = [0.1 0.05 0.01 0.005 0.001];
...
N_samples = NaN(length(h0),length(Cov_tol));
MSE = NaN(length(h0),2,length(Cov_tol));

for i = 1:length(h0)
    N_samples_i = NaN(1,length(Cov_tol));
    MSE_i = NaN(2,length(Cov_tol));
    for j = 1:length(Cov_tol)
...
save('tsb_adapt_sampled_rate_performance_data.mat');